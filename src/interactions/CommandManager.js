...
class CommandManager {
  ...
  register(command) {
    ...
    // Inject services if DI container is available
  ...
  async _handleInteraction(interaction) {
    ...
    try {
      if (interaction.isAutocomplete) {
        ...
        if (command.guards && command.guards.length > 0) {
          ...
          if (!guardResult.passed) {
            if (typeof interaction.reply === 'function' && !interaction.replied && !interaction.deferred) {
              await interaction.reply({ content: guardResult.reason, ephemeral: true }).catch(() => {});
            }
            return;
          }
        }
        ...
        // 3. Run through Middleware Pipeline then Execute
        if (this.client.middleware && this.client.middleware.stats.total > 0) {
          await this.client.middleware.execute(
            interaction,
            command.name,
            command.group || null,
            async (ctx, next) => {
